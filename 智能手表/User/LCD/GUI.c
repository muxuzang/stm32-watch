...
{  
	...
	{		
		for(pos=0;pos<size;pos++)
		{
			...
			else if(size == 64)temp=asc2_6432[num-15][pos];
	...
	}else//叠加方式
	{
		...
		{
			if(size==12)temp=asc2_1206[num][pos];//调用1206字体
			else if(size == 64)temp=asc2_6432[num-16][pos];
			else temp=asc2_1608[num][pos];		 //调用1608字体
			for(t=0;t<size/2;t++)
		    {   
				POINT_COLOR=fc;              
...
void Gui_StrCenter(u16 x, u16 y, u16 fc, u16 bc, u8 *str,u8 size,u8 mode)
{
    ...
    u16 width = len * (size / 2); // 根据字体大小计算文字宽度
    u16 x_start = (lcddev.width - width) / 2; // 计算居中的起始x位置
    u16 x_end = x_start + width - 1;

    // 清除文字所在的背景区域
    LCD_Fill(x_start, y, x_end, y + size - 1, bc); 

    // 显示文字
    Show_Str(x_start, y, fc, bc, str, size, mode);
} 
